...
function calltwitter(sinceID){
    ...
    //first call
    if(sinceID==0){
        T.get('search/tweets', { q: '#defineSOCIAM', count: 100 }, function(err, data, response) {
        //console.log(data)
    ...
    //calll after since ID has been set
         ...
         T.get('search/tweets', { q: '#defineSOCIAM', since_id: sinceID, count: 100}, function(err, data, response) {
        //console.log(data)
        max_id = data.search_metadata.max_id;
        updateDatabaseWithTweets(since_id,max_id,data);
